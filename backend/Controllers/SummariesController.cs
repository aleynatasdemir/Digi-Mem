...
public class SummariesController : ControllerBase
{
    ...
    public async Task<ActionResult<object>> GenerateWeeklyCollage([FromBody] WeeklyCollageRequest request)
    {
        ...
        {
            var userId = GetUserId();
            var filename = await _collageService.GenerateWeeklyCollageAsync(userId, request.WeekStart);
        ...
        {
            _logger.LogError(ex, "Error generating weekly collage");
    ...
    public async Task<ActionResult<object>> GenerateMonthlyCollage([FromBody] MonthlyCollageRequest request)
    {
        ...
        {
            var userId = GetUserId();
            var filename = await _collageService.GenerateMonthlyCollageAsync(userId, request.Year, request.Month);
        ...
        {
            _logger.LogError(ex, "Error generating monthly collage");
            return StatusCode(500, new { error = "Failed to generate collage" });
        }
    }
    ...
    public IActionResult DownloadCollage(string filename)
    {
        try
        {
            var filepath = Path.Combine(_env.WebRootPath, "collages", filename);
            
            if (!System.IO.File.Exists(filepath))
            {
                return NotFound(new { error = "Collage not found" });
            }
        ...
        {
            _logger.LogError(ex, "Error downloading collage");
            return StatusCode(500, new { error = "Failed to download collage" });
        }
    }
}
